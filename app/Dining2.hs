...
-- in these question we use global ordering to avoid dead lock.
-- so philosopher first tries to pick up the fork with the
-- lower number, then the fork with the higher number

getForks:: Int -> Int ->(Int,Int)
getForks n 1= (0,n-1)
getForks n i= if i-1 > i-2 then (i-2,i-1) else (i-1,i-2)
...
runPhilosopher v fs i= do
    ...
    forever $ do
        ...
        putMVar vr ()
        logOut' v $ "Philosopher "++ show i ++" puts right ( "++ show r++" ) fork."
        putMVar vl ()
        logOut' v $ "Philosopher "++ show i ++" puts left ( "++ show l++" ) fork."
type Forks = [MVar ()]
